...
const Navbar = () => {
    ...
    const linkClass = (isActive) => `py-1 text-gray-700 hover:text-blue-600 flex mb-3 flex-col items-center ${isActive ? "border-b-2 border-blue-500" : ""}`;

    return (<nav className="bg-white border border-gray-300">
        <div className="max-w-6xl min-h-[10vh] mx-auto px-2">
            <div className="flex justify-between items-center py-2">
                ...
                <div className={`md:flex md:items-center md:gap-6 ${open ? "block" : "hidden"}`}>
                    ...
                    <NavLink to={token ? "/home" : "/login"} onClick={(e) => {
                        if (!token) {
                            e.preventDefault();
                    ...
                    {token ? (<button onClick={() => navigate("/logout")} className="px-4 py-1 border dark:text-[#e06f6b] border-[#9d3533] hover:bg-[#DE3B37] text-[#000] hover:text-[#fff] transition duration-300 rounded">Logout</button>) : (<div className="flex gap-3 mt-3 md:mt-0"><button onClick={() => navigate("/login")} className="px-4 py-1 border border-blue-600 text-blue-600 rounded hover:bg-blue-50">Sign In</button><button onClick={() => navigate("/sign-up")} className="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Sign Up</button></div>)}
                </div>
            </div>
        </div>
    </nav>);
